...
```bash
# Patch-релиз (1.0.9 → 1.0.10, тэг v1.0.10)
./scripts/release.sh patch

# Minor-релиз (1.0.9 → 1.1.0, тэг v1.1.0)
./scripts/release.sh minor

# Major-релиз (1.0.9 → 2.0.0, тэг v2.0.0)
./scripts/release.sh major
```

Скрипт автоматически:
1. Увеличит версию согласно типу релиза
2. Обновит `__version__` в `reviewbot/__init__.py`
3. Закоммитит и запушит изменения
4. Создаст и запушит тэг с префиксом `v` (например, `v1.0.10`)
